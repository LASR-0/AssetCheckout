...
///  +-----------------------------------------------------------------+
//
//  Clone of RequestFormPage for accessories. Same five-section shape;
//  section 3 swaps the phone/number logic for the admin-configured
//  option choice, and the whole phone/number model is absent from the
//  payload.
//
//  Spec level follows the one-way implication pattern from the asset
//  form's call&text → data: requestType stores the user's OWN choice;
//  "Something else" locks the EFFECTIVE type to NON_STANDARD, and
//  unpicking it restores the user's choice.
//
//  POST goes through apiFetch (identity headers) — the accessory create
//  path is actor-gated, unlike the legacy asset POST.
///  +-----------------------------------------------------------------+
...
export default function AccessoryRequestFormPage() {
  ...
  // Home-page tiles can link here with ?categoryId= — validated and
  // applied by AccessoryTypeSelector once its category list has loaded.
        ...
        <div className="max-w-2xl mx-auto">
          <div className="mb-10">
            <div className="flex flex-row">
              <span className="material-symbols-outlined mx-3 !text-4xl"> cable </span>
              <h1 className="text-4xl font-bold flex tracking-tight mb-2">Checkout Accessory</h1>
            </div>
            <p className="text-info-light ml-5">
              Submit a request for a workplace accessory. Pick what you need and
              we'll supply the standard item for it.
            </p>
          </div>
